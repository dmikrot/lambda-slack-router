...
SlackBot.prototype.setRootCommand = function (args, desc, callback) {
  var realCallback = callback;
  var realDesc = desc;
  var realArgs = args;

  // if only 2 arguments are passed, then we're assuming no args are used for
  // this function, in which case we should shift all of the arguments down one
  ...
  this.root = realCallback;
  this.rootCommand = { args: realArgs, desc: realDesc };
};
...
SlackBot.prototype.callCommand = function (commandName, event, callback) {
  ...
  if (commandName === 'root') {
    args = ArgParser.align(this.rootCommand.args, event.args || []);
    if (args !== false) {
      modifiedEvent.args = args;
      return this.root(modifiedEvent, callback);
    }
  } else if (!Object.prototype.hasOwnProperty.call(this.commands, commandName)) {
...
SlackBot.prototype.findCommand = function (payload) {
  ...
  if (splitPayload[0] === 'help') {
    return { commandName: 'help' };
  }

  return { commandName: 'root', args: splitPayload };
};
